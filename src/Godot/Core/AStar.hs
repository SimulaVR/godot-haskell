...
module Godot.Core.AStar
       (Godot.Core.AStar._estimate_cost, Godot.Core.AStar._compute_cost,
        Godot.Core.AStar.get_available_point_id,
        Godot.Core.AStar.add_point, Godot.Core.AStar.get_point_position,
        Godot.Core.AStar.set_point_position,
        Godot.Core.AStar.get_point_weight_scale,
        Godot.Core.AStar.set_point_weight_scale,
        Godot.Core.AStar.remove_point, Godot.Core.AStar.has_point,
        Godot.Core.AStar.get_points,
        Godot.Core.AStar.get_point_connections,
        Godot.Core.AStar.connect_points,
        Godot.Core.AStar.disconnect_points,
        Godot.Core.AStar.are_points_connected, Godot.Core.AStar.clear,
        Godot.Core.AStar.get_closest_point,
        Godot.Core.AStar.get_closest_position_in_segment,
        Godot.Core.AStar.get_point_path, Godot.Core.AStar.get_id_path)
       where
import Data.Coerce
import Foreign.C
...
-- | Deletes the segment between the given points.
disconnect_points ::
                    (AStar :< cls, Object :< cls) => cls -> Int -> Int -> IO ()
...
are_points_connected ::
                       (AStar :< cls, Object :< cls) => cls -> Int -> Int -> IO Bool
...
-- | Returns the id of the closest point to [code]to_position[/code]. Returns -1 if there are no points in the points pool.
get_closest_point ::
                    (AStar :< cls, Object :< cls) => cls -> Vector3 -> IO Int
