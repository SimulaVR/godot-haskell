...
module Godot.Api.Types
       (HasBaseClass(..), GlobalConstants(..),
        EditorNavigationMeshGenerator(..), Geometry(..),
        EditorSelection(..), UndoRedo(..), JSON(..),
        EditorFileSystemDirectory(..), JSONRPC(..), ResourceLoader(..),
        Physics2DServer(..), Physics2DServerSW(..), CameraServer(..),
        ARVRServer(..), EditorVCSInterface(..), Engine(..),
        Physics2DDirectBodyState(..), Physics2DDirectBodyStateSW(..),
        InputMap(..), TranslationServer(..), Performance(..),
        Physics2DDirectSpaceState(..), Input(..), InputDefault(..),
        VisualServer(..), VisualScriptEditor(..),
        PhysicsDirectBodyState(..), BulletPhysicsDirectBodyState(..),
        AudioServer(..), JavaClassWrapper(..), Reference(..), Thread(..),
        UPNP(..), SkinReference(..), KinematicCollision2D(..),
        GDScriptFunctionState(..), Resource(..), OccluderPolygon2D(..),
        ShortCut(..), StyleBox(..), StyleBoxEmpty(..), StyleBoxTexture(..),
        StyleBoxFlat(..), StyleBoxLine(..), Texture(..), ProxyTexture(..),
        ImageTexture(..), LargeTexture(..), StreamTexture(..),
        CurveTexture(..), ViewportTexture(..), MeshTexture(..),
        GradientTexture(..), NoiseTexture(..), AtlasTexture(..),
        AnimatedTexture(..), CameraTexture(..), Translation(..),
        PHashTranslation(..), NavigationPolygon(..), Script(..),
        PluginScript(..), VisualScript(..), NativeScript(..), GDScript(..),
        Animation(..), PolygonPathFinder(..), RichTextEffect(..),
        OpenSimplexNoise(..), Image(..), TextureLayered(..), Texture3D(..),
        TextureArray(..), DynamicFontData(..), Gradient(..), BitMap(..),
        Sky(..), PanoramaSky(..), ProceduralSky(..),
        AnimationNodeStateMachineTransition(..), Theme(..), Shape(..),
        BoxShape(..), HeightMapShape(..), RayShape(..), SphereShape(..),
        PlaneShape(..), ConvexPolygonShape(..), ConcavePolygonShape(..),
        CapsuleShape(..), CylinderShape(..), Curve2D(..),
        AnimationNodeStateMachinePlayback(..), CryptoKey(..), TileSet(..),
        AudioBusLayout(..), MultiMesh(..), GIProbeData(..),
        MeshLibrary(..), ButtonGroup(..), InputEvent(..),
        InputEventAction(..), InputEventWithModifiers(..),
        InputEventMouse(..), InputEventMouseButton(..),
        InputEventMouseMotion(..), InputEventGesture(..),
        InputEventMagnifyGesture(..), InputEventPanGesture(..),
        InputEventKey(..), InputEventJoypadMotion(..),
        InputEventJoypadButton(..), InputEventScreenTouch(..),
        InputEventScreenDrag(..), InputEventMIDI(..), Mesh(..),
        PrimitiveMesh(..), SphereMesh(..), PointMesh(..), QuadMesh(..),
        PlaneMesh(..), CylinderMesh(..), CapsuleMesh(..), CubeMesh(..),
        PrismMesh(..), ArrayMesh(..), Font(..), BitmapFont(..),
        DynamicFont(..), EditorSettings(..), World2D(..), World(..),
        SpriteFrames(..), PackedDataContainer(..), Curve3D(..),
        GDNativeLibrary(..), CubeMap(..), AudioEffect(..),
        AudioEffectReverb(..), AudioEffectCompressor(..),
        AudioEffectRecord(..), AudioEffectAmplify(..),
        AudioEffectLimiter(..), AudioEffectEQ(..), AudioEffectEQ10(..),
        AudioEffectEQ21(..), AudioEffectEQ6(..), AudioEffectPanner(..),
        AudioEffectPitchShift(..), AudioEffectDelay(..),
        AudioEffectFilter(..), AudioEffectHighPassFilter(..),
        AudioEffectLowPassFilter(..), AudioEffectHighShelfFilter(..),
        AudioEffectLowShelfFilter(..), AudioEffectBandLimitFilter(..),
        AudioEffectNotchFilter(..), AudioEffectBandPassFilter(..),
        AudioEffectChorus(..), AudioEffectSpectrumAnalyzer(..),
        AudioEffectStereoEnhance(..), AudioEffectDistortion(..),
        AudioEffectPhaser(..), PackedScene(..), NavigationMesh(..),
        Shader(..), VisualShader(..), Environment(..), X509Certificate(..),
        EditorSpatialGizmoPlugin(..), BakedLightmapData(..), Curve(..),
        VideoStream(..), VideoStreamTheora(..), VideoStreamWebm(..),
        VideoStreamGDNative(..), Shape2D(..), CircleShape2D(..),
        CapsuleShape2D(..), RayShape2D(..), LineShape2D(..),
        SegmentShape2D(..), RectangleShape2D(..), ConvexPolygonShape2D(..),
        ConcavePolygonShape2D(..), Material(..), SpatialMaterial(..),
        CanvasItemMaterial(..), ParticlesMaterial(..), ShaderMaterial(..),
        Skin(..), VisualScriptNode(..), VisualScriptYield(..),
        VisualScriptBasicTypeConstant(..), VisualScriptClassConstant(..),
        VisualScriptFunctionCall(..), VisualScriptWhile(..),
        VisualScriptEmitSignal(..), VisualScriptVariableGet(..),
        VisualScriptIterator(..), VisualScriptExpression(..),
        VisualScriptIndexGet(..), VisualScriptSelf(..),
        VisualScriptDeconstruct(..), VisualScriptVariableSet(..),
        VisualScriptBuiltinFunc(..), VisualScriptConstant(..),
        VisualScriptLocalVarSet(..), VisualScriptGlobalConstant(..),
        VisualScriptResourcePath(..), VisualScriptIndexSet(..),
        VisualScriptConstructor(..), VisualScriptFunction(..),
        VisualScriptYieldSignal(..), VisualScriptSequence(..),
        VisualScriptOperator(..), VisualScriptSelect(..),
        VisualScriptSubCall(..), VisualScriptInputAction(..),
        VisualScriptSceneNode(..), VisualScriptLists(..),
        VisualScriptComposeArray(..), VisualScriptSceneTree(..),
        VisualScriptLocalVar(..), VisualScriptPropertyGet(..),
        VisualScriptTypeCast(..), VisualScriptComment(..),
        VisualScriptPropertySet(..), VisualScriptCondition(..),
        VisualScriptReturn(..), VisualScriptEngineSingleton(..),
        VisualScriptSwitch(..), VisualScriptPreload(..),
        VisualScriptCustomNode(..), VisualScriptMathConstant(..),
        AudioStream(..), AudioStreamMicrophone(..),
        AudioStreamOGGVorbis(..), AudioStreamRandomPitch(..),
        AudioStreamSample(..), AudioStreamGenerator(..), TextFile(..),
        AnimationNode(..), AnimationNodeBlend2(..),
        AnimationNodeOutput(..), AnimationNodeTimeSeek(..),
        AnimationRootNode(..), AnimationNodeAnimation(..),
        AnimationNodeStateMachine(..), AnimationNodeBlendTree(..),
        AnimationNodeBlendSpace2D(..), AnimationNodeBlendSpace1D(..),
        AnimationNodeAdd3(..), AnimationNodeTransition(..),
        AnimationNodeBlend3(..), AnimationNodeAdd2(..),
        AnimationNodeOneShot(..), AnimationNodeTimeScale(..),
        VisualShaderNode(..), VisualShaderNodeGroupBase(..),
        VisualShaderNodeExpression(..),
        VisualShaderNodeGlobalExpression(..), VisualShaderNodeCompare(..),
        VisualShaderNodeVectorRefract(..),
        VisualShaderNodeTransformCompose(..),
        VisualShaderNodeVectorSmoothStep(..),
        VisualShaderNodeVec3Constant(..), VisualShaderNodeOutput(..),
        VisualShaderNodeTransformFunc(..), VisualShaderNodeScalarOp(..),
        VisualShaderNodeVectorScalarSmoothStep(..),
        VisualShaderNodeVectorScalarMix(..),
        VisualShaderNodeScalarInterp(..),
        VisualShaderNodeScalarConstant(..),
        VisualShaderNodeVectorDerivativeFunc(..), VisualShaderNodeIs(..),
        VisualShaderNodeVectorScalarStep(..), VisualShaderNodeInput(..),
        VisualShaderNodeTexture(..), VisualShaderNodeTransformVecMult(..),
        VisualShaderNodeVectorDistance(..), VisualShaderNodeUniform(..),
        ...
        VisualShaderNodeVec3Uniform(..), VisualShaderNodeVectorInterp(..),
        VisualShaderNodeVectorLen(..),
        VisualShaderNodeScalarSmoothStep(..),
        VisualShaderNodeFaceForward(..), VisualShaderNodeColorConstant(..),
        VisualShaderNodeFresnel(..), VisualShaderNodeVectorFunc(..),
        VisualShaderNodeDeterminant(..), VisualShaderNodeScalarFunc(..),
        VisualShaderNodeTransformMult(..), VisualShaderNodeColorOp(..),
        VisualShaderNodeBooleanConstant(..),
        VisualShaderNodeDotProduct(..),
        VisualShaderNodeVectorDecompose(..), VisualShaderNodeIf(..),
        VisualShaderNodeTransformConstant(..), VisualShaderNodeCustom(..),
        VisualShaderNodeVectorCompose(..),
        VisualShaderNodeScalarDerivativeFunc(..),
        VisualShaderNodeTransformDecompose(..), VisualShaderNodeSwitch(..),
        VisualShaderNodeScalarSwitch(..), VisualShaderNodeColorFunc(..),
        VisualShaderNodeScalarClamp(..), VisualShaderNodeOuterProduct(..),
        VisualShaderNodeVectorOp(..), VisualShaderNodeVectorClamp(..),
        VisualShaderNodeCubeMap(..), PhysicsMaterial(..), EditorScript(..),
        Mutex(..), VisualScriptFunctionState(..), SceneTreeTimer(..),
        JSONParseResult(..), ARVRInterface(..), MobileVRInterface(..),
        ARVRInterfaceGDNative(..), AnimationTrackEditPlugin(..),
        SurfaceTool(..), EditorExportPlugin(..), ResourceImporter(..),
        EditorImportPlugin(..), SpatialGizmo(..), EditorSpatialGizmo(..),
        MeshDataTool(..), EncodedObjectAsID(..), GDNative(..),
        EditorFeatureProfile(..), PhysicsShapeQueryResult(..),
        PCKPacker(..), XMLParser(..), AudioEffectInstance(..),
        AudioEffectSpectrumAnalyzerInstance(..), EditorScenePostImport(..),
        ResourceFormatLoader(..), ResourceFormatLoaderCrypto(..),
        MultiplayerAPI(..), EditorResourceConversionPlugin(..),
        AudioStreamPlayback(..), AudioStreamPlaybackResampled(..),
        AudioStreamGeneratorPlayback(..), ResourceFormatSaver(..),
        ResourceFormatSaverCrypto(..), HashingContext(..), ConfigFile(..),
        AStar2D(..), WebRTCPeerConnection(..),
        WebRTCPeerConnectionGDNative(..), PacketPeer(..),
        WebRTCDataChannel(..), WebRTCDataChannelGDNative(..),
        PacketPeerGDNative(..), PacketPeerUDP(..),
        NetworkedMultiplayerPeer(..), MultiplayerPeerGDNative(..),
        NetworkedMultiplayerENet(..), WebRTCMultiplayer(..),
        WebSocketMultiplayerPeer(..), WebSocketClient(..),
        WebSocketServer(..), WebSocketPeer(..), PacketPeerStream(..),
        PackedDataContainerRef(..), Expression(..), TCP_Server(..),
        CharFXTransform(..), KinematicCollision(..), File(..), FuncRef(..),
        UPNPDevice(..), CameraFeed(..), TriangleMesh(..),
        EditorSceneImporter(..), EditorSceneImporterAssimp(..),
        WeakRef(..), ResourceInteractiveLoader(..),
        Physics2DTestMotionResult(..), RegEx(..), Semaphore(..),
        JavaClass(..), Marshalls(..), RandomNumberGenerator(..),
        EditorInspectorPlugin(..), Physics2DShapeQueryParameters(..),
        StreamPeer(..), StreamPeerSSL(..), StreamPeerBuffer(..),
        StreamPeerTCP(..), StreamPeerGDNative(..), RegExMatch(..),
        EditorResourcePreviewGenerator(..), SceneState(..),
        SpatialVelocityTracker(..), HTTPClient(..),
        Physics2DShapeQueryResult(..), AStar(..), Directory(..),
        Crypto(..), PhysicsShapeQueryParameters(..), Node(..),
        CanvasItem(..), Node2D(..), AnimatedSprite(..),
        CollisionObject2D(..), Area2D(..), PhysicsBody2D(..),
        KinematicBody2D(..), StaticBody2D(..), RigidBody2D(..),
        ParallaxLayer(..), RayCast2D(..), Bone2D(..), Light2D(..),
        TileMap(..), Position2D(..), TouchScreenButton(..),
        VisibilityNotifier2D(..), VisibilityEnabler2D(..), Sprite(..),
        Camera2D(..), MeshInstance2D(..), AudioStreamPlayer2D(..),
        Line2D(..), Polygon2D(..), Particles2D(..), YSort(..),
        NavigationPolygonInstance(..), LightOccluder2D(..),
        CollisionShape2D(..), CanvasModulate(..), CPUParticles2D(..),
        MultiMeshInstance2D(..), PathFollow2D(..), Path2D(..),
        BackBufferCopy(..), Navigation2D(..), RemoteTransform2D(..),
        CollisionPolygon2D(..), Joint2D(..), GrooveJoint2D(..),
        DampedSpringJoint2D(..), PinJoint2D(..), Skeleton2D(..),
        Control(..), LineEdit(..), VideoPlayer(..), Container(..),
        TabContainer(..), SplitContainer(..), HSplitContainer(..),
        VSplitContainer(..), ScrollContainer(..), EditorInspector(..),
        PanelContainer(..), ScriptEditor(..), MarginContainer(..),
        EditorProperty(..), ViewportContainer(..), GraphNode(..),
        CenterContainer(..), GridContainer(..), BoxContainer(..),
        HBoxContainer(..), VBoxContainer(..), ColorPicker(..),
        ItemList(..), Tree(..), ColorRect(..), NinePatchRect(..),
        Range(..), TextureProgress(..), SpinBox(..), ScrollBar(..),
        VScrollBar(..), HScrollBar(..), EditorSpinSlider(..), Slider(..),
        VSlider(..), HSlider(..), ProgressBar(..), ReferenceRect(..),
        Separator(..), HSeparator(..), VSeparator(..), BaseButton(..),
        LinkButton(..), TextureButton(..), Button(..),
        ColorPickerButton(..), ToolButton(..), CheckBox(..),
        OptionButton(..), MenuButton(..), CheckButton(..), Popup(..),
        WindowDialog(..), AcceptDialog(..), ConfirmationDialog(..),
        FileDialog(..), ScriptCreateDialog(..), EditorFileDialog(..),
        PopupDialog(..), PopupMenu(..), PopupPanel(..), TextureRect(..),
        Label(..), Panel(..), Tabs(..), RichTextLabel(..), GraphEdit(..),
        TextEdit(..), AnimationTreePlayer(..), EditorInterface(..),
        ResourcePreloader(..), Tween(..), CanvasLayer(..),
        ParallaxBackground(..), SkeletonIK(..), EditorPlugin(..),
        InstancePlaceholder(..), AnimationPlayer(..),
        EditorResourcePreview(..), Timer(..), AnimationTree(..),
        AudioStreamPlayer(..), HTTPRequest(..), Spatial(..), RayCast(..),
        ARVROrigin(..), GridMap(..), ProximityGroup(..),
        VisibilityNotifier(..), VisibilityEnabler(..), SpringArm(..),
        CollisionShape(..), VehicleWheel(..), Skeleton(..),
        VisualInstance(..), GeometryInstance(..), CSGShape(..),
        CSGPrimitive(..), CSGTorus(..), CSGMesh(..), CSGBox(..),
        CSGSphere(..), CSGPolygon(..), CSGCylinder(..), CSGCombiner(..),
        Particles(..), SpriteBase3D(..), AnimatedSprite3D(..),
        Sprite3D(..), MultiMeshInstance(..), ImmediateGeometry(..),
        MeshInstance(..), SoftBody(..), CPUParticles(..), Light(..),
        OmniLight(..), DirectionalLight(..), SpotLight(..),
        BakedLightmap(..), ReflectionProbe(..), GIProbe(..),
        RootMotionView(..), Camera(..), ARVRCamera(..),
        InterpolatedCamera(..), ClippedCamera(..), Path(..),
        ARVRAnchor(..), NavigationMeshInstance(..), PathFollow(..),
        CollisionObject(..), Area(..), PhysicsBody(..), RigidBody(..),
        VehicleBody(..), StaticBody(..), PhysicalBone(..),
        KinematicBody(..), ARVRController(..), Navigation(..),
        AudioStreamPlayer3D(..), CollisionPolygon(..), BoneAttachment(..),
        Position3D(..), Listener(..), RemoteTransform(..), Joint(..),
        ConeTwistJoint(..), PinJoint(..), SliderJoint(..), HingeJoint(..),
        Generic6DOFJoint(..), WorldEnvironment(..), EditorFileSystem(..),
        Viewport(..), IP(..), IP_Unix(..), ClassDB(..), PhysicsServer(..),
        BulletPhysicsServer(..), PhysicsDirectSpaceState(..),
        ARVRPositionalTracker(..), MainLoop(..), SceneTree(..),
        ResourceSaver(..), JavaScript(..), TreeItem(..), OS(..),
        ProjectSettings(..))
       where
import Data.Coerce
import Foreign.C
